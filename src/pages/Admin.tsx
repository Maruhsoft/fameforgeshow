...
const Admin: React.FC<AdminProps> = ({ data, onDataChange }) => {
  ...
  const [isStreamSettingsOpen, setIsStreamSettingsOpen] = React.useState(false);
  ...
  const {
    ...
    defaultValues: data.currentStream,
  });
  ...
  const onSubmitStream = (formData: StreamFormData) => {
    const newData = { ...data };
    newData.currentStream = formData;
    onDataChange(newData);
    ...
    >
      <div className="mb-8">
        ...
        <p className="text-gray-400">
          Manage contestants, seasons, and live stream settings
        </p>
      </div>
      ...
      <div className="bg-gray-800 rounded-xl p-6 mb-8">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-xl font-bold mb-2">Live Stream Settings</h2>
            <p className="text-gray-400">
              Current Status:{' '}
              <span className={data.currentStream.streamStatus ? 'text-green-400' : 'text-red-400'}>
                {data.currentStream.streamStatus ? 'Live' : 'Offline'}
              </span>
            </p>
            <p className="text-gray-400">
              Next Stream: {format(new Date(data.currentStream.nextStreamDate), 'PPP p')}
            </p>
          </div>
          <Button onClick={() => setIsStreamSettingsOpen(true)}>
      ...
      <Dialog
        isOpen={isStreamSettingsOpen}
        onClose={() => setIsStreamSettingsOpen(false)}
        title="Live Stream Settings"
      >
        <form onSubmit={handleSubmitStream(onSubmitStream)} className="space-y-6">
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">YouTube URL</label>
              <Input
                {...registerStream('youtubeUrl')}
                error={streamErrors.youtubeUrl?.message}
            ...
            <div>
              ...
              <Input
                {...registerStream('videoId')}
                error={streamErrors.videoId?.message}
                placeholder="Enter YouTube video ID"
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-2">Next Stream Date</label>
              <Input
                type="datetime-local"
                {...registerStream('nextStreamDate')}
                error={streamErrors.nextStreamDate?.message}
              />
            </div>
            <div className="flex items-center space-x-2">
              <input
                type="checkbox"
                {...registerStream('streamStatus')}
                className="rounded bg-gray-700 border-gray-600 text-purple-600 focus:ring-purple-500"
              />
              <label className="text-sm font-medium">Stream is Live</label>
            </div>
          </div>
          <div className="flex justify-end space-x-4">
            <Button
              type="button"
              variant="secondary"
              onClick={() => setIsStreamSettingsOpen(false)}
            >
              Cancel
            </Button>
            <Button type="submit">
      ...
      >
        <form onSubmit={handleSubmitContestant(onSubmitContestant)} className="space-y-6">
          <div className="space-y-4">
            <input type="hidden" {...registerContestant('id')} />
            <div>
